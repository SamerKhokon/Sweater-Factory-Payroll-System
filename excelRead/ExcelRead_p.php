...
for($i=0;$i<count($num_sheet);$i++)
{
	...
	for ($j = 2; $j <= $num_rows; $j++) {
		...
		$SpouseN = $data->sheets[$i]['cells'][$j][5];
		$DofBirth = $data->sheets[$i]['cells'][$j][6];
		$Age = $data->sheets[$i]['cells'][$j][7];
		//$birthCer = $data->sheets[$i]['cells'][$j][9];
		$MotherNs	=explode('.',$MotherN);
	
		
	echo $sql = "insert into TEMP_EMP_P(section,cardno,name,gross,joindate,grade,designation) values ('".$member_id."','".$Member_name."','".$chan_mID."','".$MotherNs[0]."',to_date('".$SpouseN."','dd/mm/YYYY'),'".$DofBirth."','".$Age."')";
	echo $j.'<br>';
 //$res = mysql_query($sql);
