...
$sql	="select  COMPANY_NAME from TBL_COMPANY_INFO where ID=".$_SESSION['company_id']."";
...
<div class="grid_12">
	...
	/*$payroll_main_menu_all 	=$_SESSION['payroll_main_menu_all'];
	$payroll_sub1_menu_all 	=$_SESSION['payroll_sub1_menu_all'];
	$payroll_sub2_menu_all 	=$_SESSION['payroll_sub2_menu_all'];
	*/
    ...
    while($row = mysqli_fetch_array($stid)) 
    {
		...
		$main_menu_id=$row['ID'];
		$sql_sub="select * from TBL_PAYROLL_SUB_MENU1 where MAIN_MENU_ID=$main_menu_id  and ID in (".$payroll_user_sub_menu.")";			
		...
		{  
			...
			echo "<li><a href=\"?pagetitle=$sub_menu_content&menu_id=$main_menu_id&sm_id=$PAYROLL_SUB_MENU_ID\" >$sub_menu_name</a>";
			$k=0;
		...
		while($row_sub1 = mysqli_fetch_array($stid2)) 	
				...
				echo "<li><a href=\"?pagetitle=$sub_menu2_content&menu_id=$main_menu_id&sm_id=$sub_menu2_id\" >$sub_menu2_name</a></li>";
				}
			if($k>0)
			echo "</ul>";
