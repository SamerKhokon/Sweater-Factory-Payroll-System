...
$sampleArray2=trim($_GET['sampleArray2']);
...
<?php
	...
	if($section_id!='*')
	{
		$xsql=" and a.SECTION_ID in (".$sampleArray2.")";
	}
	$status_str	="";
	if($type_id==1)
		$status_str=" and a.STATUS=$type_id";
	if($type_id==2)
		$status_str=" and a.STATUS!=1";
		
	$sql ="select (select SEC_NAME from TBL_PAY_SECTION_INFO where COMPANY_ID=$company_id and ID=a.SECTION_ID) as SEC_NAME,CARD_ID,NAME,to_char(JOIN_DATE,'dd.mm.yyyy') as jdate,DESIGNATION,SECTION_ID,(select BLOCK_NAME from TBL_PAY_SECTION_BLOCK where COMPANY_ID=$company_id and ID=a.BLOCK_ID) as BLOCK_ID,GRADE,BASIC,HOUSE_RENT,MEDICAL,GROSS,cast(substr(a.CARD_ID,2,9) as number) as pp from TBL_PAY_EMP_PROFILE a where COMPANY_ID=$company_id  ".$xsql."  ".$status_str." order by pp,SECTION_ID,BLOCK_ID";		
