...
function fixedEmp_paysleep($card_id,$namef,$grade,$designation,$basic_view,$h_rent_view,$medical_view,$actual_salary_view,$total_w_day,$atten,$leave,$late_pre,$lunch_out,$abs_days,$atn_bon_view,$holiday,$holi_amnt_view,$ot,$ot_rate,$ot_amount,$total_amnt_view,$fest_v_view,$advance_view,$net_pay_view,$dateid,$dateidT,$str,$section_id,$net_pay,$basic_gain,$stamp,$leave_amnt,$casual_l,$sick_l,$total_amount,$gov_holiday,$govt_holi_amnt,$block_id,$other_amnt,$abs_amnt,$company_id,$food_amnt,$amount_cv)
{
	...
	if($company_id==2)
	{
		$str_add	='<b>Top Style Sweater Ltd.</b><br><br>';	
	}
	...
	*/
	...
	<tbody>
	<tr>
		<td align="center" colspan="20">'.$str_add.''.$str.' Date '.$dateid.' To '.$dateidT.'</td>
	</tr>
	<tr>
	...
	:&nbsp;'.getSectionName($section_id).'<br>Line:'.getBlockName($block_id,$section_id).'<br />Desig/<img src="bangla_img/designation1.png" height="15px;">:&nbsp;'.$designation.'</td>
			...
			<td colspan="3">Med&nbsp;:'.$medical_view.'<br><img src="bangla_img/medical.png" height="14px;">:&nbsp;'.$app->get_unicode_bn_num($medical_view).'<br>Food&nbsp;:'.$food_amnt.'</td>
		...
		<tr>
			...
			<td rowspan="2">Abs<br>Amnt</td>
			<td rowspan="2">Fest<br>Lev</td>
			<td rowspan="2">Fest<br>Amnt</td>
        ...
        <tr>
			  ...
			  <td>'.make_comma($abs_amnt).'<br><br>'.$app->get_unicode_bn_num(make_comma($abs_amnt)).'</td>
...
<?php
	...
	b.WORKS_DAY,b.TOTAL_ATTEND,b.LEAVE,b.LATE_PRESENT,b.LUNCH_OUT,b.OT,b.HOLY_DAY,b.ADVANCE ,a.GROSS,a.HOUSE_RENT,a.MEDICAL,b.CASUAL,b.SICK,b.GOVT_HOLI,a.BLOCK_ID,b.OTHER_AMNT,(select TO_DATE('".$dateidT."', 'MM/DD/YY')-(select TO_DATE(to_char(JOIN_DATE,'MM/DD/YY'),'MM/DD/YY') from TBL_PAY_EMP_PROFILE where ID=a.ID) FROM dual) as day_diff,cast(substr(a.CARD_ID,2,9) as number) as pp  
	...
	
	{
		...
		$abs_amnt2			=round($abs_amnt);
		//$basic_gain	=round((($basic/$total_w_day)*$ap_days),2);
		
		//$basic_gain			=round($gross-$abs_amnt-$food_amnt_d);  02-02-2014
		...
		$total_amount			=round($basic_gain+$fest_v+$ot_amount+$atn_bon+$other_amnt);
