...
<table border="0" cellpadding="0" cellspacing="0" align="center"  class="details" style="font-family:solaimanlipi, Arial, Helvetica, sans-serif;"> 
	<tbody><tr>
			...
			<td colspan="5">Section/<img src="bangla_img/division.png" height="13px;">
	:&nbsp;'.getSectionName($section_id).'<br>Line:'.getBlockName($block_id,$section_id).'<br />Desig/<img src="bangla_img/designation1.png" height="15px;">:&nbsp;'.$designation.'</td>
			...
			<td colspan="2">H Rent&nbsp;:'.$h_rent_view.'<br><img src="bangla_img/house_rent1.png" height="14px;">:&nbsp;'.$app->get_unicode_bn_num($h_rent_view).'<br>Con amt&nbsp;:'.$amount_cv.'</td>
...
<?php
	...
	*/
	...
	b.WORKS_DAY,b.TOTAL_ATTEND,b.LEAVE,b.LATE_PRESENT,b.LUNCH_OUT,b.OT,b.HOLY_DAY,b.ADVANCE ,a.GROSS,a.HOUSE_RENT,a.MEDICAL,b.CASUAL,b.SICK,b.GOVT_HOLI,a.BLOCK_ID,b.OTHER_AMNT,(select TO_DATE('".$dateidT."', 'MM/DD/YY')-(select TO_DATE(to_char(JOIN_DATE,'MM/DD/YY'),'MM/DD/YY') from TBL_PAY_EMP_PROFILE where ID=a.ID) FROM dual) as day_diff,a.SECTION_ID as SECTION_ID,cast(substr(a.CARD_ID,2,9) as number) as pp  
	...
	
	{
		...
		*/	
		...
		$total_amnt				=round($gross+$ot_amount+$atn_bon+$holi_amnt+$other_amnt+$food_amnt+$amount_cv);
		...
		//$total_amount			=round($basic_gain+$fest_v+$ot_amount+$atn_bon+$other_amnt+$food_amnt+$amount_cv);
