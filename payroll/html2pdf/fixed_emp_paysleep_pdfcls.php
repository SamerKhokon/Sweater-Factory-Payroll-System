...
function fixedEmp_paysleep($card_id,$name,$grade,$designation,$basic_view,$h_rent_view,$medical_view,$actual_salary_view,$total_w_day,$atten,$leave,$late_pre,$lunch_out,$abs_days,$atn_bon_view,$holiday,$holi_amnt_view,$ot,$ot_rate,$ot_amount,$total_amnt_view,$fest_v_view,$advance_view,$net_pay_view,$dateid,$dateidT,$str,$section_id,$net_pay,$basic_gain,$stamp)
{

	$app = new app_utility();
	$mtest2 = new salary_cal();
	
	return '<table width="90%" style="width:90%;font-family:solaimanlipi;" align="center"><tr><td style="width:220px;">&nbsp;</td><td>'.$mtest2->tbl_header_fun1($dateid,$dateidT,$str).'</td><td>Report Generated Date '.date('m/d/Y').'</td></tr></table>
<table border="0" cellpadding="0" cellspacing="0" align="center"  class="details" style="font-family:vrinda , Arial, Helvetica, sans-serif;"> 
	...
	:&nbsp;'.$mtest2->getSectionName($section_id).'</td>
            ...
            <td rowspan="2">House<br />Rent<br><img src="bangla_img/house_rent1.png" ></td>
            <td rowspan="2">Medical<br><img src="bangla_img/medical.png"></td>
            <td rowspan="2">Gross<br><img src="bangla_img/total.png"></td>
            <td colspan="6">Attendence Information</td>
			<td rowspan="2">Basic<br>Gain<br><img src="bangla_img/basic_gn.png"></td>
            ...
            <td rowspan="2">Fest<br><img src="bangla_img/occation.png"></td>
            <td rowspan="2">Advance<br><img src="bangla_img/advance.png"></td>
			<td rowspan="2">Stamp<br><img src="bangla_img/stamp.png"></td>
            <td rowspan="2">Net Pay<br>Amount<br />Amount<br><img src="bangla_img/grant_total.png"></td>
		...
		<tr>
            ...
            <td>Leave<br><img src="bangla_img/holiday1.png"></td>
            <td>L.Pre<br><img src="bangla_img/late.png"></td>
            <td>L.Out<br><img src="bangla_img/lunch_out.png"></td>
        ...
        <tr>
              ...
              <td>'.$total_w_day.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($total_w_day)).'</td>
              <td>'.$atten.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($atten)).'</td>
              <td>'.$leave.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($leave)).'</td>
              <td>'.$late_pre.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($late_pre)).'</td>
              <td>'.$lunch_out.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($lunch_out)).'</td>
              <td>'.$abs_days.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($abs_days)).'</td>
			  <td>'.$mtest2->make_comma($basic_gain).'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($basic_gain)).'</td>
              <td>'.$atn_bon_view.'<br><br>'.$app->get_unicode_bn_num($atn_bon_view).'</td>
              <td>'.$holiday.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($holiday)).'</td>
              <td>'.$holi_amnt_view.'<br><br>'.$app->get_unicode_bn_num($holi_amnt_view).'</td>
              <td>'.$ot.'<br><br>'.$app->get_unicode_bn_num($ot).'</td>
              <td>'.$ot_rate.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($ot_rate)).'</td>
              <td>'.$ot_amount.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($ot_amount)).'</td>
              <td>'.$fest_v_view.'<br><br>'.$app->get_unicode_bn_num($fest_v_view).'</td>
              <td>'.$advance_view.'<br><br>'.$app->get_unicode_bn_num($advance_view).'</td>
			  <td>'.$stamp.'<br><br>'.$app->get_unicode_bn_num($mtest2->make_comma($stamp)).'</td>
...
<?php
	...
	
	{
		...
		$medical_view	=$mtest->make_comma($medical);
		...
		$holi_amnt		=$mtest->fn_holiamnt($holiday,$basic,$total_w_day);
