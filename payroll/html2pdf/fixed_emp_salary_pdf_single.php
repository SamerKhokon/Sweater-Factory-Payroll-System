...
$card_id	= trim(isset($_GET['card_id'])?$_GET['card_id']:'');
...
<table border="0" cellpadding="0" cellspacing="0" align="center"  class="details"> 
    <thead>
    	...
    	<tr>
        	<th colspan="9"><b>Section:<?php echo  getSectionName($section_id); ?></b></th><th colspan="10"></th><th colspan="6"><b>Line:<?php echo getBlockName($block_id,$section_id); ?></b></th><th colspan="5">Page Number:&nbsp;[[page_cu]]/[[page_nb]]</th>
        </tr>
...
//to_char(b.MONTH_YEAR,'mm/yyyy')='$month_year'	
	...
	a.SECTION_ID='$section_id' and a.COMPANY_ID=$company_id and a.STATUS='1' and a.CARD_ID ='$card_id' and to_char(b.MONTH_YEAR,'mm/yyyy') between '$month_year1' and '$month_year2' ".$xsql."   
	order by pp";
	...
	b.WORKS_DAY,b.TOTAL_ATTEND,b.LEAVE,b.LATE_PRESENT,b.LUNCH_OUT,b.OT,b.HOLY_DAY,b.ADVANCE ,a.GROSS,a.HOUSE_RENT,a.MEDICAL,b.CASUAL,b.SICK,a.BLOCK_ID,a.GRADE,b.GOVT_HOLI,b.EXTRA_OT,(select TO_DATE('".$dateidT."', 'MM/DD/YY')-(select TO_DATE(to_char(JOIN_DATE,'MM/DD/YY'),'MM/DD/YY') from TBL_PAY_EMP_PROFILE where ID=a.ID) FROM dual) as day_diff,cast(substr(a.CARD_ID,2,9) as number) as pp
	...
	
	{
				...
				*/
				...
				$actual_gross_amnt	=round($basic_gain+$fest_v+$ot_amount+$atn_bon);
 ...
 <tr style="font-weight:bold;">
 	<td colspan="4">Total Employee:<?php echo $j; ?></td><td colspan="5"></td><td><?php //echo make_comma($actual_salary_total); ?></td><td colspan="7"></td><td><?php echo make_comma($leave_amnt_total); ?></td><td><?php echo make_comma($atn_bon_total); ?></td><td><?php //echo make_comma($govt_holi_total); ?></td><td><?php echo make_comma($govt_holi_amnt_total); ?></td><td><?php  echo make_comma($ot_total); ?></td><td></td><td><?php echo make_comma($ot_amnt_total); ?></td><td><?php echo make_comma($gross_amnt_total); ?></td><td><?php echo make_comma($advance_total); ?></td><td><?php echo make_comma($stamp_total); ?></td><td><?php echo make_comma($net_pay_total); ?></td><td></td>
 </tr>
 <tr style="font-weight:bold;">
