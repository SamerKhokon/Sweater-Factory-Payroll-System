...
$sql ="select (select STYLE_NAME from TBL_PAY_STYLE_INFO where  ID=a.STYLE_ID and COMPANY_ID=$company_id) as STYLE_NAME,(select SIZE_NAME from TBL_PAY_SIZE_SETTING where ID=a.SIZE_ID and STYLE_ID=a.STYLE_ID and COMPANY_ID=$company_id) as SIZE_NAME,(select RATE from TBL_PAY_RATE_SETTING where STYLE_ID=a.STYLE_ID and SIZE_ID=a.SIZE_ID and COMPANY_ID=$company_id and SECTION_ID=$section_id) as RATE,sum(QUANTITY)  as QUANTITY,a.STYLE_ID,a.SIZE_ID from TBL_PAY_EMP_PRODUCTION_ISSUE a where CARD_ID='$card_id' and a.COMPANY_ID=$company_id and a.SECTION_ID=$section_id and to_char(pro_date,'mm/dd/YYYY') between '$dateid' and '$dateto' group by STYLE_ID,SIZE_ID order by STYLE_ID";
		...
		while($rs = oci_fetch_array($qstr2,OCI_BOTH+OCI_RETURN_NULLS))
		{
		  ...
		  $sql ="select  sum(QUANTITY)  as QUANTITYP from TBL_PAY_EMP_PRODUCTION c where CARD_ID='$card_id' and c.COMPANY_ID=$company_id and c.SECTION_ID=$section_id and STYLE_ID= $style_id  and SIZE_ID=$size_id and to_char(pro_date,'mm/dd/YYYY') between '$dateid' and '$dateto'";
		   $pro_stm = oci_parse($conn, $sql);
			oci_execute($pro_stm);
		    while($row = oci_fetch_array($pro_stm,OCI_BOTH+OCI_RETURN_NULLS))
			{
				$pro_r_quwantity=$row[0];
			}
			$pro_r_quwantity_total+=$pro_r_quwantity;
	...
	echo   "<tr>
		   ...
		   <td>$total_quantity</td><td>$pro_r_quwantity_total</td>
		   </tr>";  
 }     
  echo '</tbody></table>';
echo tbl_footer_A4();
//<td>$total_amnt</td>	<td>$rate</td>
