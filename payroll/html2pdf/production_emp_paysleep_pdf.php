...
function productionEmp_paysleep($card_id,$name,$grade,$designation,$basic_view,$h_rent_view,$medical_veiw,$actual_salary_view,$total_w_day,$atten,$leave,$late_pre,$lunch_out,$abs_days,$basic_gain,$atn_bon_view,$holiday,$holi_amnt_view,$ot,$ot_rate,$ot_amount,$total_amnt_view,$fest_v_view,$advance_view,$produc_amntT,$produc_bonus,$total_v,$net_pay_view,$dateid,$dateidT,$str,$section_id,$net_pay,$stamp,$leave_amnt,$casual_l,$sick_l,$total_amnt,$block_id2,$gov_holiday,$govt_holi_amnt,$other_amnt,$abs_amnt,$company_id,$food_amnt,$amount_cv,$vt,$no_work_amnt,$no_work,$ot,$advance)
{
	...
	if($company_id==1)
	{
		$str_add	='<b>Ocean Gate Sweaters Ltd.</b><br>Hazi Nizam Plaza, (2nd-5th) Floor.Palli Bidyut, Nabinagor, Savar<br>';
	}
	if($company_id==2)
	{
	...
	*/
	...
	<tbody>
	<tr>
			<td align="center" colspan="21">'.$str_add.''.$str.'<br>'.$dateid.' To '.$dateidT.'</td>
	</tr>
	<tr>
			...
			<td colspan="5">Section/<img src="bangla_img/section.png" height="13px;">:&nbsp;'.getSectionName($section_id).'<br>Line:'.getBlockName($block_id2,$section_id).'<br />Desig/<img src="bangla_img/designation1.png" height="15px;">:&nbsp;'.$designation.'</td>
			
			<td colspan="3">Basic&nbsp;:'.$basic_view.'<br><img src="bangla_img/basic1.png" height="15px;">:&nbsp;'.$app->get_unicode_bn_num($basic_view).'</td>
		...
		<tr>
			...
			<td rowspan="2">No<br />Wrk</td>
			<td rowspan="2">No<br />Wrk<br>Amt</td>
			<td rowspan="2">OT<br />Hr</td>
			<td rowspan="2">OT<br />Rt</td>
            <td rowspan="2">OT<br />Amt</td>
        ...
        <tr>
			  ...
			  <td>'.make_comma($no_work).'<br><br>'.$app->get_unicode_bn_num(make_comma($no_work)).'</td>
			  <td>'.make_comma($no_work_amnt).'<br><br>'.$app->get_unicode_bn_num(make_comma($no_work_amnt)).'</td>
			  <td>'.make_comma($ot).'<br><br>'.$app->get_unicode_bn_num(make_comma($ot)).'</td>
			  
			  <td>'.make_comma($ot_rate).'<br><br>'.$app->get_unicode_bn_num(make_comma($ot_rate)).'</td>
			  <td>'.make_comma($ot_amount).'<br><br>'.$app->get_unicode_bn_num(make_comma($ot_amount)).'</td>
			  ...
			  <td>'.$advance.'<br><br>'.$app->get_unicode_bn_num(make_comma($advance)).'</td>
		...
		<tr>
			<td colspan="21" align="left">'.convert_number($net_pay).'</td>
		</tr>
 </tbody></table>............................................................................................................................................................................................................................................................................................................................................................';
}
...
b.WORKS_DAY,b.TOTAL_ATTEND,b.LEAVE,b.LATE_PRESENT,b.LUNCH_OUT,b.OT,b.HOLY_DAY,b.ADVANCE,a.GROSS,a.HOUSE_RENT,a.MEDICAL,b.CASUAL,b.SICK,b.BLOCK_ID,b.GOVT_HOLI,b.OTHER_AMNT,(select TO_DATE('".$dateidT."', 'MM/DD/YY')-(select TO_DATE(to_char(JOIN_DATE,'MM/DD/YY'),'MM/DD/YY') from TBL_PAY_EMP_PROFILE where ID=a.ID) FROM dual) as day_diff,cast(substr(a.CARD_ID,1,9) as number) as pp,b.NO_WORK as NO_WORK      
...
while($row = oci_fetch_array($qstr, OCI_BOTH+OCI_RETURN_NULLS))
{
			...
			$ot_rate=round(($basic/26 * .70));
			$ot_amount=round($ot*$ot_rate);
			
			$ot_amnt_total 		+=$ot_amount;
			
			$no_work			=$row['NO_WORK'];
			$no_work_amnt		=round(fn_holiamnt($no_work,$gross,$total_w_day));
			...
			$produc_chk			=round(($produc_amnt['production_amnt']+$produc_bonus+$fest_v+$ot_amount+$atn_bon+$leave_amnt+$govt_holi_amnt+$other_amnt+$no_work_amnt));
			...
			echo productionEmp_paysleep($card_id,$name,$grade,$designation,$basic_view,$h_rent_view,$medical_veiw,$actual_salary_view,$total_w_day,$atten,$leave,$late_pre,$lunch_out,$abs_days,$basic_gain,$atn_bon_view,$holiday,$holi_amnt_view,$ot,$ot_rate,$ot_amount,$total_amnt_view,$fest_v_view,$advance_view,$produc_amntT,$produc_bonus,$total_v,$net_pay_view,$dateidT,$dateidT,$str,$section_id,$net_pay,$stamp,$leave_amnt,$casual_l,$sick_l,$total_amnt,$block_id,$gov_holiday,$govt_holi_amnt,$other_amnt,$abs_amnt,$company_id,$food_amnt,$amount_cv,$vt2,$no_work_amnt,$no_work,$ot,$advance);
