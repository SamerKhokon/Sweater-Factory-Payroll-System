...
<table border="0" cellpadding="0" cellspacing="0" align="center"  class="details"> 
    <thead>
    	<tr>
        	<th colspan="9"><b>Section:<?php echo  getSectionName($section_id) ?></b></th><th colspan="10"></th><th colspan="8"><b>Line:<?php echo getBlockName($block_id,$section_id); ?></b></th><th colspan="3">Page Number:&nbsp;[[page_cu]]/[[page_nb]]</th>
        </tr>
        <tr>
...
//to_char(b.MONTH_YEAR,'mm/yyyy')='$month_year'	
	...
	a.SECTION_ID='$section_id' and a.STATUS='1' and a.CARD_ID='$card_id'  and to_char(b.MONTH_YEAR,'mm/yyyy') between '$month_year1' and '$month_year2' ".$xsql."   
	order by pp";
	...
	
	{
				...
				$actual_produc_chk	=($produc_amnt['production_amnt']+$produc_bonus+$fest_v+$actual_ot_amnt+$atn_bon+$leave_amnt+$govt_holi_amnt);
	...
	?>				
		...
		if($row = oci_fetch_array($qstr, OCI_BOTH+OCI_RETURN_NULLS))
		{
			if($block_id2!='')
			{
				...
				$dateid,$leave_amnt_total,$govt_holi_amnt_total,$total_v_all);
			}
		}	
	 	else
		{
...
try
{
	$html2pdf = new HTML2PDF('L', 'Legal', 'fr',array(2, 10, 0,0));
