...
$sql ="select distinct SECTION_ID,
		...
		while($rs = oci_fetch_array($qstr,OCI_BOTH+OCI_RETURN_NULLS))
		  ...
		  echo "<tr><td></td><td></td><td>Total</td><td>$total_quantity</td><td>Total</td><td>$total_amnt</td></tr>";
		}
			 $grand_total_amnt	+=$grtotal_amnt;
			 $grand_total_qty	+=$grtotal_quantity;
			 echo "<tr><td colspan='2'></td><td align='right'>Total Qty</td><td>$grand_total_qty</td><td align='right'>Grand Total</td><td>$grand_total_amnt</td></tr>"; 
}	    
  echo '</tbody></table>';
echo tbl_footer_A4();
