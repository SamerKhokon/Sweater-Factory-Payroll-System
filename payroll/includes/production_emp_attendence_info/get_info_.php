...
echo '!@#$';
$sql	="select TOTAL_ATTEND,LATE_PRESENT,NO_WORK,OT,ADVANCE,LEAVE,LUNCH_OUT,GOVT_HOLI,CASUAL,ANUAL,SICK,ID  from TBL_PAY_EMP_ATTEN_INFO where CARD_ID='$card_no' and SECTION_ID=$section_id	and COMPANY_ID=$company_id and to_char(MONTH_YEAR,'mm/yyyy')='$month_year'";
$stid	= oci_parse($conn, $sql);
oci_execute($stid);
if($row = oci_fetch_array($stid, OCI_BOTH+OCI_RETURN_NULLS))
{
