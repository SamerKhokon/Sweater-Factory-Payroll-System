...
$year	=explode("/",$datepicker);


/*
$sql	="select count(ID) from TBL_PAY_STYLE_INFO where upper(STYLE_NAME)='$style' and COMPANY_ID=$company_id and to_char(ORDER_DATE,'YYYY')='$year[2]'";
*/

$sql	="select count(ID) from TBL_PAY_STYLE_INFO where upper(STYLE_NAME)='$style' and COMPANY_ID=$company_id";
...
if($mids==0)
{
	$sql	="insert into TBL_PAY_STYLE_INFO(COMPANY_ID,STYLE_NAME,QUENTITY,BUYER_NAME,ORDER_QTY,UNIT_PRICE,MERCH_NAME,GAUGE,MACHINE_QTY,SHIPMENT_DATE,BUYER_ST_NAME,SHIP_STATUS,ENTRY_BY,ORDER_DATE) values('".$company_id."','".$style."','".$add_qty."','".$buyer_name."','".$quantity."','".$u_price."','".$merchend_name."','".$gauge."','".$mach_qty."',to_date('$shipmentd','mm/dd/yyyy'),'".$bstyle_name."','".$shipment_st."','$usr_id',to_date('$datepicker','mm/dd/yyyy'))";
...
else
{
	...
	{
		$style_id	=$row[0];
		$sql="update TBL_PAY_STYLE_INFO set QUENTITY='$add_qty',BUYER_NAME='$buyer_name',ORDER_QTY='$quantity',UNIT_PRICE='$u_price',MERCH_NAME='$merchend_name',GAUGE='$gauge',MACHINE_QTY='$mach_qty',SHIPMENT_DATE=to_date('$shipmentd','mm/dd/yyyy'),BUYER_ST_NAME='$bstyle_name',SHIP_STATUS='$shipment_st',ENTRY_BY='$usr_id',ORDER_DATE=to_date('$datepicker','mm/dd/yyyy') where ID=$style_id";
		$result	=oci_parse($conn,$sql);
		$success=oci_execute($result);
		if($success)
		{
