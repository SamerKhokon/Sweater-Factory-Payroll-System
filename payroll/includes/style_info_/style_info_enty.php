			...
			for($i=0; $i<count($sizeNMarr);$i++)
			{
				...
				if($success)
				{
					...
					if($row1 = oci_fetch_array($stid, OCI_BOTH))
					{
						...
						{
						$sql	="insert into TBL_PAY_RATE_SETTING(SECTION_ID,COMPANY_ID,STYLE_ID,SIZE_ID,RATE) values('".$res[0]."','".$company_id."','".$row[0]."','".$row1[0]."',0)";
						$result	=oci_parse($conn,$sql);
						$success=oci_execute($result);
						}
			...
			if($mid==0)
			{
				...
				if($success)
				{
					...
					if($row1 = oci_fetch_array($stid, OCI_BOTH+OCI_RETURN_NULLS))
					{
						...
						{
							$sql	="insert into TBL_PAY_RATE_SETTING(SECTION_ID,COMPANY_ID,STYLE_ID,SIZE_ID,RATE) values('".$res[0]."','".$company_id."','".$style_id."','".$row1[0]."',0)";
							$result	=oci_parse($conn,$sql);
							$success=oci_execute($result);
						}
