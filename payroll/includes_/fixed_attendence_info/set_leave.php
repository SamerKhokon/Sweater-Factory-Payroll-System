...
{
	$sql	="select * from TBL_PAY_LEAVE_INFO where CARD_ID='$card_id' and SECTION_ID=$section_id and COMPANY_ID=$company_id and to_char(MONTH_YEAR,'mm/yyyy')='$month_year'";
...
$sqlcasu = "select sum(CASUAL) from TBL_PAY_LEAVE_INFO where CARD_ID='$card_id' and COMPANY_ID=$company_id  and SECTION_ID=$section_id  and to_char(MONTH_YEAR,'yyyy')='2013' group by CARD_ID";

$sqlannual = "select sum(ANUAL) from TBL_PAY_LEAVE_INFO where CARD_ID='$card_id' and COMPANY_ID=$company_id and  SECTION_ID=$section_id  and to_char(MONTH_YEAR,'yyyy')='2013' group by CARD_ID";

$sqlsick = "select sum(SICK) from TBL_PAY_LEAVE_INFO where CARD_ID='$card_id' and COMPANY_ID=$company_id and SECTION_ID=$section_id  and to_char(MONTH_YEAR,'yyyy')='2013' group by CARD_ID";
