...
<?php
	...
	if($section_id=='')
	  ...
	  $str = "select * from tbl_pay_emp_profile where COMPANY_ID=$company_id and section_id in
			(select ID from TBL_PAY_SECTION_INFO where sec_type_id=(select id from tbl_pay_section_type where cat='$type')) ".$section_id_str."  order by cast(substr(card_id,2,9) as number)";
...
<div id="demo">
	...
	<tbody>
<?php
		while($res = oci_fetch_array($emp_info_str,OCI_BOTH+OCI_RETURN_NULLS)) {
	    $slno            = $res['ID'];
		...
		<tr class="gradeA">
		 ...
		 <td><?php echo $emp_id;?></td>
		  <td><a href="index.php?pagetitle=employee_profile_display&menu_id=22&sm_id=3&slno=<?php echo $res['ID'];?>" >
<input type="button" id="fixed_salary_info_display_detail_button" class="btn btn-teal" value="details"/>
</a>
<a href="index.php?pagetitle=production_base_emp_info_display_edit&menu_id=22&sm_id=4&slno=<?php echo $slno;?>">
		  <input type="button" id="salary_setting_edit_button" class="btn btn-teal" value="edit"/>
		  </a><input type="button" id="btn_del" name="<?php echo $res['ID']; ?>" class="btn btn-teal"  value="Delete" /></td>		 
		</tr>		
<?php	}?>
</tbody>
</tfoot>	  
