...
$sql	="select ID,TOTAL_ATTEND,LATE_PRESENT,NO_WORK,OT,ADVANCE,LEAVE,LUNCH_OUT,GOVT_HOLI,OTHER_AMNT from TBL_PAY_EMP_ATTEN_INFO where CARD_ID='$cardno' and SECTION_ID=$section_id	and COMPANY_ID=$company_id and to_char(MONTH_YEAR,'mm/yyyy')='$month_year'";
$stid	= oci_parse($conn, $sql);
oci_execute($stid);
if($row = oci_fetch_array($stid, OCI_BOTH+OCI_RETURN_NULLS)) 
{
	
	$sql	="update TBL_PAY_EMP_ATTEN_INFO set TOTAL_ATTEND='$atten',LATE_PRESENT='$l_present',LEAVE='$leave',OT='$ot',NO_WORK='$no_work',ADVANCE='$advanced',LUNCH_OUT='$lunch_out',CASUAL='$casual',ANUAL='$annual',SICK='$sick',GOVT_HOLI='$govt_holi',OTHER_AMNT='$other_amnt',HOLY_DAY='$friday'  where ID='".$row[0]."'";
	...
	if($success)
	{
		...
		if($row1 = oci_fetch_array($stid, OCI_BOTH+OCI_RETURN_NULLS)) 
		{
			...
			$resul	=oci_parse($conn,$sql);
			$succe=oci_execute($resul);
			if($succe)
...
else
{
	...
	$sql	="insert into TBL_PAY_EMP_ATTEN_INFO(SECTION_ID,COMPANY_ID,CARD_ID,BLOCK_ID,MONTH_YEAR,WORKS_DAY,TOTAL_ATTEND,LATE_PRESENT,LEAVE,OT,NO_WORK,HOLY_DAY,ADVANCE,LUNCH_OUT,CASUAL,ANUAL,SICK,GOVT_HOLI,OTHER_AMNT) values('".$section_id."','".$company_id."','".$cardno."','".$block_name."',to_date('".$datepicker."','mm-dd-yyyy'),'".$total_day_of_month."','".$atten."','".$l_present."','".$leave."','".$ot."','".$no_work."','".$friday."','".$advanced."','".$lunch_out."','".$casual."','".$annual."','".$sick."','".$govt_holi."','".$other_amnt."')";
	$result	=oci_parse($conn,$sql);
	$success=oci_execute($result);
	if($success)
		{
