...
$sizeQty =0;
$usr_id	=23;
$sql	="select distinct(SIZE_ID),SECTION_ID,STYLE_ID from TBL_PAY_EMP_PRODUCTION where SIZE_ID is not null";
...
while($row = oci_fetch_array($stid, OCI_BOTH+OCI_RETURN_NULLS)) 
{
	...
	$size=$row[0];
	$section=$row[1];
	$sql2	="select * from TBL_PAY_RATE_SETTING where STYLE_ID=$style and SIZE_ID=$size and SECTION_ID=$section";
